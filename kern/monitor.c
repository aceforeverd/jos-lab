...
int mon_time(int argc, char **argv, struct Trapframe *tf) {
    ...
    for (int i = 0; i < NCOMMANDS; i++) {
        if (strcmp(cmd, commands[i].name) == 0) {
            found = 1;
            uint32_t start_time_high, start_time_low, end_time_low,
                end_time_high;
            uint64_t start, end;
            __asm __volatile(
                "rdtsc\n\t"
...
static uint32_t read_ret_pointer(uint32_t ebp) {
    ...
    __asm __volatile("movl (%1), %0" : "=r"(ret_pointer) : "r"(ebp + 4));
    return ret_pointer;
}

static uint32_t read_prev_ebp(uint32_t curr_ebp) {
    uint32_t prev_ebp;
    __asm __volatile("movl (%1), %0" : "=r"(prev_ebp) : "r"(curr_ebp));
    return prev_ebp;
}

static uint32_t read_parameter(uint32_t ebp, uint32_t offset) {
    uint32_t param;
    __asm __volatile("movl (%1), %0" : "=r"(param) : "r"(ebp + offset));
    return param;
}
...
int mon_backtrace(int argc, char **argv, struct Trapframe *tf) {
    ...
    while (ebp != 0x0) {
        struct Eipdebuginfo info = {"unknow", 0, "unknow", 7, 0, 0};
        int result = debuginfo_eip(eip, &info);

        cprintf("eip %08x ebp %08x args %08x %08x %08x %08x %08x\n", eip, ebp,
                read_parameter(ebp, 8), read_parameter(ebp, 12),
                read_parameter(ebp, 16), read_parameter(ebp, 20),
                read_parameter(ebp, 24));
