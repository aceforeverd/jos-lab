...
static int check_addr(void *addr) {
	return ((uintptr_t) addr <= UTOP ) && ( (uintptr_t) addr % PGSIZE == 0 );
}
...
static int check_env(struct Env *e) {
	envid_t cur_id = curenv->env_id;
	return (e->env_id == cur_id) || (e->env_parent_id == cur_id);
}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
    ...
    if (!check_addr(srcva) || !check_addr(dstva)) {
        return -E_INVAL;
    }

    pte_t *pte;
    struct Page *p = page_lookup(src_env->env_pgdir, srcva, &pte);
    if (!p) {
        return -E_INVAL;
    }
    ...
    ret = page_insert(dst_env->env_pgdir, p, dstva, perm);
    /* 0 or -E_NO_MEM */
    return ret;
}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
    ...
    if ((uintptr_t) srcva < UTOP) {
        ...
        if ((uintptr_t) e->env_ipc_dstva < UTOP) {
            int r = page_insert(e->env_pgdir, p, e->env_ipc_dstva, perm);
            if (r < 0) {
                return r;
            }
            e->env_ipc_perm = perm;
